...
-- Group Members :  Suresh Kumar Singh, Shashi  & Versha Verma 
...
SELECT g.genre as Genre, COUNT(m.id) AS Total_Num_of_Movies
FROM genre g
INNER JOIN movie m
ON g.movie_id = m.id
GROUP BY genre
...
(
SELECT
movie_id, COUNT(genre) AS no_of_movies
FROM genre
GROUP BY movie_id
HAVING no_of_movies=1
)
SELECT COUNT(movie_id) AS Total_Number_of_Movies
...
SELECT * FROM avg_rating_rank_summary WHERE movie_rank<=10;
...
SELECT  median_rating, COUNT(id) as movies_released
FROM movie m
INNER JOIN ratings r
ON m.id = r.movie_id
...
SELECT country, sum(total_votes) AS Total_Votes_Count
FROM movie as m
...
(
SELECT genre,
YEAR, title as movie_name, worlwide_gross_income,
DENSE_RANK() OVER(PARTITION BY year ORDER BY worlwide_gross_income DESC) AS movie_rank
FROM movie m
INNER JOIN genre g
ON m.id = g.movie_id
WHERE genre IN(SELECT genre FROM top3_genre) and worlwide_gross_income like "$%"
)
SELECT *
FROM top_movies
WHERE movie_rank<=5;
...
WITH movie_date_info_summary AS
(
